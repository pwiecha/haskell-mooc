...
prettyPrint size queens = prettyPrint' size queens (1,1)
  where
    prettyPrint' size queens coord
      | fst coord == size+1 = []
      | snd coord == size+1 = '\n' : prettyPrint' size queens (nextRow coord)
      | otherwise = placeElement coord queens : prettyPrint' size queens (nextCol coord)
    placeElement coord queens
      | elem coord queens = 'Q'
      | otherwise = '.'
...
danger c [] = False
danger c (q:qs) = checkForDangerZone c q || danger c qs -- laziness?
  where checkForDangerZone c q = any (\f -> f c q) [sameRow, sameCol, sameDiag, sameAntidiag]
...
prettyPrint2 size queens = prettyPrint' size queens (1,1)
  where
    prettyPrint' size queens coord
      | fst coord == size+1 = []
      | snd coord == size+1 = '\n' : prettyPrint' size queens (nextRow coord)
...
fixFirst n (q:qs)
  | checkValid n q = case danger q qs of
      True -> fixFirst n ((nextCol q):qs)
      False -> Just (q:qs)
  | otherwise = Nothing
  where checkValid n (x,y) = x <= n && y <= n

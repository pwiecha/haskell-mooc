...
fmap2 fn = fmap $ fmap fn -- apply fmap fn to every element

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 fn = fmap $ fmap $ fmap fn
...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (LNode v lst) = LNode (f v) (fmap f lst)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode v v' lst) = TwoNode (f v ) (f v') (fmap f lst)
...
count i = foldr (\e a-> countCompare i e + a) 0
  where
  countCompare i e
    | i == e = 1
    | otherwise = 0
...
inBoth xs ys = foldr (\x res -> if x `elem` ys then x:res else res) [] xs
------------------------------------------------------------------------------
...
instance Foldable List where
  ...
  foldr f init (LNode val rest) = f val (foldr f init rest)
...
instance Foldable TwoList where
  ...
  foldr f init (TwoNode a a' rest) = f a $ f a' $ foldr f init rest
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node val lTree rTree) = sumTree lTree <> val <> sumTree rTree

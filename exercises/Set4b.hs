...
maxHelper currItem prevItem
  | currItem > prevItem = currItem
  | otherwise = prevItem
-- or simply max function
...
slHelper currItem (prevSum, prevLen) = (prevSum+currItem, prevLen+1)
...
largestHelper currItem [] = [currItem] -- init
-- (prevItem:prevItems) version
largestHelper currItem (prevItem:prevItems)
  | currItem > prevItem = [currItem] -- replace
  | currItem == prevItem = currItem:prevItem:prevItems -- prepend
  | otherwise = prevItem:prevItems
{--
largestHelper currItem prevLst 
  | currItem > prevHead = [currItem] -- replace
  | currItem == prevHead = currItem:prevLst -- prepend
  | otherwise = prevLst
    where prevHead = head prevLst
--}
...
headHelper currItem _ = Just currItem
